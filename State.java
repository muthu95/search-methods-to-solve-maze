...
public class State {
	...
	private boolean isSafe(int xx, int yy, Maze maze) {
		if(xx >= 0 && xx < maze.getNoOfRows() && yy >= 0 && yy < maze.getNoOfCols())
			return true;
		return false;
	}
	...
	public ArrayList<State> getSuccessors(boolean[][] explored, Maze maze) {
		...
		for(i=0; i<4; i++) {
			xx = getX() + dx[i];
			yy = getY() + dy[i];
			if(isSafe(xx, yy, maze) && !explored[xx][yy] && maze.getSquareValue(xx, yy) != '%') {
				neighbors.add(new State(new Square(xx, yy), this, getGValue()+1, getDepth()+1));
			}
		}
		return neighbors;
	}
